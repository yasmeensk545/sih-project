import { Scheme, UserProfile, EligibilityResult, EligibilityStatus } from '../types';
...
export function evaluateEligibility(
  ...
  if (elig.states && elig.states.length > 0) {
    ...
    const matchesState = isPanIndia || elig.states.some(s => s.toLowerCase() === profile.state.toLowerCase());
  ...
  if (elig.education && elig.education.length > 0) {
    const matchesEdu = elig.education.some(edu => {
  ...
  if (elig.additional_criteria && elig.additional_criteria.length > 0) {
    elig.additional_criteria.forEach(crit => {
